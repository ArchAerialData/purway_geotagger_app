...
def _build_service(fake_http: FakeJsonHttpClient) -> WindWeatherAutofillService:
    return WindWeatherAutofillService(
        weather_client=NwsObservationClient(http_client=fake_http),
        metar_client=AviationWeatherMetarClient(http_client=fake_http, max_history_hours=1_000_000),
...
def test_build_autofill_maps_required_fields_from_nws_observations() -> None:
    ...
    service = _build_service(fake_http)
...
def test_build_autofill_backfills_missing_fields_from_aviationweather_metar_before_open_meteo() -> None:
    fake_http = FakeJsonHttpClient(
        {
            f"{NWS_BASE_URL}/points/29.7604,-95.3698": {
            ...
            AWC_METAR_URL: [
                {
                    "icaoId": "KHOU",
                    "obsTime": 1770404400,  # 2026-02-06T19:00:00Z
                    "reportTime": "2026-02-06T19:00:00.000Z",
                    "temp": 11.0,
                    "wdir": 210,
                    "wspd": 6,
                    "wgst": 18,
                ...
                {
                    "icaoId": "KHOU",
                    "obsTime": 1770418800,  # 2026-02-06T23:00:00Z
                    "reportTime": "2026-02-06T23:00:00.000Z",
                    "temp": 13.0,
                    "wdir": 300,
                    "wspd": 8,
                    "wgst": 22,
    ...
    assert result.start.direction == "SSW"
    assert result.start.gust_mph == 21
    assert result.start.speed_mph == 10
    assert result.start.temp_f == 52
    assert "Wind direction not reported" not in " | ".join(result.start.warnings)
    assert "Wind gust not reported" not in " | ".join(result.start.warnings)
    assert result.verification_url is not None
    assert AWC_METAR_URL in result.verification_url
    assert any("Filled missing values from AviationWeather METAR" in warning for warning in result.warnings)


def test_build_autofill_returns_partial_row_when_field_missing_and_backfill_unavailable() -> None:
    fake_http = FakeJsonHttpClient(
        {
            f"{NWS_BASE_URL}/points/29.7604,-95.3698": {
                "properties": {"observationStations": f"{NWS_BASE_URL}/gridpoints/HGX/54,97/stations"}
    ...
    assert result.verification_url is not None
    assert "/stations/KHOU/observations" in result.verification_url
    assert any("Open-Meteo backfill was unavailable" in warning for warning in result.warnings)
...
def test_build_autofill_falls_back_to_aviationweather_metar_when_nws_missing() -> None:
    fake_http = FakeJsonHttpClient(
        {
            f"{NWS_BASE_URL}/points/29.7604,-95.3698": {
                "properties": {"observationStations": f"{NWS_BASE_URL}/gridpoints/HGX/54,97/stations"}
    ...
    assert result.start.gust_mph == 21
    assert result.end.direction == "NNW"
    assert result.end.speed_mph == 13
    assert result.end.gust_mph == 25
    assert result.verification_url is not None
    assert AWC_METAR_URL in result.verification_url
    assert any("used AviationWeather METAR observations" in warning for warning in result.warnings)
